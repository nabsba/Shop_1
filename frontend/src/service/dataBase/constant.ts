...
const SQL_OBJECT: { [key: string]: TObjectSql } = {
	ALL_SHOES: {
		...
		mode: 'select',
		object: {},
		sql: 'SELECT product.product_id,product.name, color_id FROM product INNER JOIN product_has_color ON product.product_id=product_has_color.product_id order by product_id desc limit 20;',
	},
};
